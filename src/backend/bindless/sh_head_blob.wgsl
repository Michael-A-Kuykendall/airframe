...
struct HeadBlobParams {
    ...
    weight_off: u32,   // byte offset of output.weight inside the GGUF blob
...
fn main_lm_head(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    let weight_off = params.weight_off;
    let dim        = params.dim;
    var dot        = 0.0f;
    ...
    if (params.quant_type == 14u) { // Q6_K  (210 bytes/block, 256 elems/block)
        ...
        let bpr       = dim / 256u;
        let row_start = weight_off + idx * bpr * 144u;
        for (var b = 0u; b < bpr; b++) {
        ...
        for (var col = 0u; col < dim; col++) {
            let w_byte = weight_off + (idx * dim + col) * 2u;
            dot += act_in[col] * dequant_f16_at(w_byte);
        }
    } else if (params.quant_type == 0u) { // F32
        for (var col = 0u; col < dim; col++) {
            let w_idx = weight_off / 4u + idx * dim + col;
            dot += act_in[col] * bitcast<f32>(read_blob(w_idx));
        }
    } else { // Q4_0 (default / fallback)  (18 bytes/block, 32 elems/block)
        let bpr          = dim / 32u;
        let row_start_b  = weight_off + idx * bpr * 18u;
        for (var b = 0u; b < bpr; b++) {
            ...
            for (var i = 0u; i < 32u; i++) {
                ...
                let qs_word  = read_blob(byte_idx / 4u);
