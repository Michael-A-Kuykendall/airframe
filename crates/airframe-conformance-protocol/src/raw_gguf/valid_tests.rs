//! Tests for raw GGUF reader — valid input parsing.

use crate::raw_gguf::{ModelIdentity, RawGgufReader};
use std::fs::File;
...
fn raw_gguf_valid_minimal() {
    ...
    assert!(identity.file_hash.len() == 64); // SHA256 hex
    assert!(identity.tensor_directory_hash.len() == 64);
}

#[test]
fn raw_gguf_valid_multiple_tensors() {
    let dir = tempdir().unwrap();
    ...
    assert_eq!(identity.tensors.len(), 2);
    ...
    // Check storage upper bounds
    // tensor 1: offset = data_start, upper bound = tensor 2 offset
    // tensor 2: offset = data_start + tensor1_size, upper bound = file end
    assert_eq!(t1.storage_upper_bound, t2.offset);
    assert_eq!(
        t2.storage_upper_bound,
        std::fs::metadata(&path).unwrap().len()
    );
}

#[test]
fn model_identity_hashes_stable() {
    let dir = tempdir().unwrap();
    let path = create_minimal_gguf(dir.path());
