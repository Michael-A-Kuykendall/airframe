...
mod tests {
    ...
    fn all_tier1_facts_have_alpha_keys() {
        let facts = vec![
            InferenceFact::LayerOutput { layer_idx: 0, position: 1, rms_bits: 0, checksum: 0 },
            InferenceFact::FinalLogits { position: 1, rms_bits: 0, checksum: 0 },
            InferenceFact::OutputToken { step: 0, token_id: 1 },
        ];
        for f in &facts {
            assert!(alpha_key_of(f).is_some(), "Tier 1 fact must have alpha key: {:?}", f);
        }
    }
    ...
    fn tier3_consequents_have_no_alpha_key() {
        let facts = vec![
            ...
            InferenceFact::TriggerCandleCompare { rms_bits: 0, checksum: 0 },
        ];
        for f in &facts {
            assert!(alpha_key_of(f).is_none(), "Tier 3 fact must not have alpha key: {:?}", f);
        }
    }

    #[test]
    fn distinct_variants_have_distinct_keys() {
        let k1 = alpha_key_of(&InferenceFact::LayerOutput { layer_idx: 0, position: 0, rms_bits: 0, checksum: 0 });
        let k2 = alpha_key_of(&InferenceFact::FinalLogits { position: 0, rms_bits: 0, checksum: 0 });
        assert_ne!(k1, k2, "LayerOutput and FinalLogits must have distinct alpha keys");
    }
}
