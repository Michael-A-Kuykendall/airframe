...
#[test]
fn layer_window_plan_half_split_resolves_oversized_span() {
    clear_invariant_log();

    use airframe::backend::bindless::pipeline::plan_layer_half_windows;

    // airframe-eyn regression case: a layer whose FULL tensor span straddles
    // more than BLOB_BINDING_SLOTS chunks cannot use one window, but its
    // attention and FFN halves each fit within 8 slots. This mirrors
    // qwen2.5-coder-14b layer 11 (full span = 9 slots) and gemma-2-9b layer 7
    // (full span = 10 slots).
    //
    // Layout: attention tensors span chunks 1..4 (4 slots), FFN tensors span
    // chunks 5..8 (4 slots). The UNION spans chunks 1..8 = 8 slots — so to
    // force a real >8-slot union we widen it: attention chunks 1..5 and FFN
    // chunks 6..9 gives a union of 9 slots (1..9) while each half is 5 slots.
    let span = (CHUNK_WORDS_128_MIB as u64) * 4; // bytes per chunk
    let base_byte = 0u64;

    // Attention tensors at chunk 1..5: abs = base + 4 + k*span for k=1..5
    let attn_abs: Vec<u64> = (1..=5).map(|k| 4 + k * span).collect();
    // FFN tensors at chunk 6..9: abs = base + 4 + k*span for k=6..9
    let ffn_abs: Vec<u64> = (6..=9).map(|k| 4 + k * span).collect();
    ...
    use airframe::backend::bindless::pipeline::relative_packed_offset;
    let pack = |abs: u64| relative_packed_offset(abs, base_byte).expect("packed offset");

    offs.attn_norm = pack(attn_abs[0]);
    offs.attn_q = pack(attn_abs[1]);
    offs.attn_k = pack(attn_abs[2]);
    offs.attn_v = pack(attn_abs[3]);
    offs.attn_out = pack(attn_abs[4]);
    offs.ffn_norm = pack(ffn_abs[0]);
    offs.ffn_gate = pack(ffn_abs[1]);
    offs.ffn_up = pack(ffn_abs[2]);
    offs.ffn_down = pack(ffn_abs[3]);
    // leave ffn_abs[4] (chunk 9) unassigned; union is chunks 1..8 = 8 slots;
    // to guarantee >8 union, put ffn_up/ffn_down such that FFN spans 6..9 = 4
    // slots but the union 1..9 = 9 slots. We assigned ffn_norm(6), gate(7),
    // up(8), down(9) -> FFN spans chunks 6..9 (4 slots). Attention spans 1..5
    // (5 slots). Union = 1..9 = 9 slots > 8. Each half <= 5 <= 8.

    let blob_base_words = (base_byte / 4) as u32;

    // Sanity: the WHOLE-layer planner must reject this span (proves the test
    // exercises the eyn failure).
    use airframe::backend::bindless::pipeline::plan_layer_window;
    let whole = plan_layer_window(&offs, blob_base_words, CHUNK_WORDS_128_MIB, 16);
    assert!(
        whole.is_err(),
        "whole-layer span must exceed 8 slots (eyn case): {:?}",
        whole
    );

    // The half-window planner must resolve both halves.
    let (attn, ffn) = plan_layer_half_windows(&offs, blob_base_words, CHUNK_WORDS_128_MIB, 16)
        .expect("half windows must plan");
    let (attn_win, attn_base) = attn.expect("attention tensors present");
    let (ffn_win, ffn_base) = ffn.expect("ffn tensors present");

    assert!(
        attn_win.slot_count <= 8 && ffn_win.slot_count <= 8,
        "each half must fit 8 slots: attn={} ffn={}",
        attn_win.slot_count,
        ffn_win.slot_count
    );
    assert_eq!(attn_win.start_chunk, 1);
    assert_eq!(ffn_win.start_chunk, 6);

    // Each half's rebased base must reconstruct its own tensors.
    for (&abs, &packed) in attn_abs.iter().zip(
        [
            offs.attn_norm,
            offs.attn_q,
            offs.attn_k,
        ...
        let local = (packed / 2).wrapping_add(attn_base);
        let (slot, offset) = attn_win
            .absolute_to_local(attn_win.window_base_words() + local)
            .expect("attn tensor in attn window");
        assert!(slot < attn_win.slot_count);
        assert_eq!(
            attn_win.local_to_absolute(slot, offset),
            (abs / 4) as u32,
            "attn tensor at {abs} must reconstruct through the attn window"
        );
    }

    // FFN half round-trip (uses ffn_base and ffn_win).
    for (&abs, &packed) in ffn_abs
        .iter()
        .zip([offs.ffn_norm, offs.ffn_gate, offs.ffn_up, offs.ffn_down].iter())
    {
        let local = (packed / 2).wrapping_add(ffn_base);
        let (slot, offset) = ffn_win
            .absolute_to_local(ffn_win.window_base_words() + local)
            .expect("ffn tensor in ffn window");
        assert!(slot < ffn_win.slot_count);
        assert_eq!(
            ffn_win.local_to_absolute(slot, offset),
            (abs / 4) as u32,
            "ffn tensor at {abs} must reconstruct through the ffn window"
        );
    }

    contract_test(
        "layer_window_plan_half_split_resolves_oversized_span",
        &["Word index must be within buffer bounds"],
    );
}
