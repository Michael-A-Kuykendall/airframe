...
fn extract_gguf_metadata<P: AsRef<Path>>(path: P) -> Result<HashMap<String, String>, Box<dyn std::error::Error>> {
...
fn spec_from_gguf_metadata(metadata: &HashMap<String, String>) -> Result<ModelSpec, Box<dyn std::error::Error>> {
    use airframe::core::spec::{ModelArch, GgufFileType};
    
    let arch_str = metadata.get("general.architecture")
        .map(|s| s.as_str())
        .unwrap_or("llama");
    
    let n_vocab = metadata.get("tokenizer.ggml.tokens")
        .and_then(|s| s.parse::<usize>().ok())
        .unwrap_or(32000);
    
    let n_embd = metadata.get("llama.embedding_length")
        .and_then(|s| s.parse::<usize>().ok())
        .ok_or("Missing llama.embedding_length")?;
    
    let n_layer = metadata.get("llama.block_count")
        .and_then(|s| s.parse::<usize>().ok())
        .ok_or("Missing llama.block_count")?;
    
    let n_head = metadata.get("llama.attention.head_count")
        .and_then(|s| s.parse::<usize>().ok())
        .ok_or("Missing llama.attention.head_count")?;
    
    let n_head_kv = metadata.get("llama.attention.head_count_kv")
        .and_then(|s| s.parse::<usize>().ok())
        .unwrap_or(n_head);
    
    let ff_dim = metadata.get("llama.feed_forward_length")
        .and_then(|s| s.parse::<usize>().ok())
        .ok_or("Missing llama.feed_forward_length")?;
    
    let rms_eps = metadata.get("llama.attention.layer_norm_rms_epsilon")
        .and_then(|s| s.parse::<f32>().ok())
        .unwrap_or(1e-5);
    
    let rope_base = metadata.get("rope.freq_base")
        .and_then(|s| s.parse::<f32>().ok())
        .unwrap_or(10000.0);
    
    let rope_dim = metadata.get("rope.dimension_count")
        .and_then(|s| s.parse::<usize>().ok())
        .unwrap_or(n_embd / n_head);
    
    let n_ctx = metadata.get("context_length")
        .and_then(|s| s.parse::<usize>().ok())
        .unwrap_or(2048);
    
    // Parse file type from general.file_type
    let file_type = metadata.get("general.file_type")
...
fn generate_oracle_for_gguf<P: AsRef<Path>>(gguf_path: P) -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("[1/5] Model loaded: {} layers, {} dim", spec.n_layer, spec.n_embd);
    ...
    println!("      ✓ Processed BOS (pos 0) through all {} layers", spec.n_layer);
    ...
    let filename = gguf_path.file_stem()
        .and_then(|s| s.to_str())
        .unwrap_or("unknown");
