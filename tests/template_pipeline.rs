...
fn model_dir() -> PathBuf {
    let base =
        std::env::var("SHIMMY_TEST_MODELS").unwrap_or_else(|_| "D:/shimmy-test-models".to_string());
    PathBuf::from(base)
}
...
fn pipeline_tinyllama_renders_user_turn_with_eos() {
    ...
    let (tmpl, ctx) =
        build_context_from_gguf(&path, true).expect("failed to load TinyLlama tokenizer info");
    ...
    assert!(
        out.contains("</s>"),
        "eos_token from tokenizer; got: {out:?}"
    );
...
fn pipeline_gemma2_bos_and_turn_delimiters() {
    ...
    let (tmpl, ctx) =
        build_context_from_gguf(&path, true).expect("failed to load Gemma2 tokenizer info");
    ...
    assert!(
        out.contains("<start_of_turn>model\n"),
...
fn pipeline_phi35_eos_from_tokenizer() {
    ...
    let (tmpl, ctx) =
        build_context_from_gguf(&path, true).expect("failed to load Phi-3.5 tokenizer info");
...
fn pipeline_qwen2_injects_default_system() {
    ...
    let (tmpl, ctx) =
        build_context_from_gguf(&path, true).expect("failed to load Qwen2 tokenizer info");
...
fn pipeline_deepseek_bos_from_tokenizer_and_plain_text_style() {
    ...
    let (tmpl, ctx) =
        build_context_from_gguf(&path, true).expect("failed to load DeepSeek tokenizer info");
    ...
    assert!(
        out.contains(bos),
        "DeepSeek bos token from tokenizer; got: {out:?}"
    );
    assert!(
        out.contains("User: Hello\n\n"),
...
fn pipeline_llama32_full_header_format() {
    ...
    let (tmpl, ctx) =
        build_context_from_gguf(&path, true).expect("failed to load Llama-3.2 tokenizer info");
    ...
    assert!(
        out.contains("<|begin_of_text|>"),
        "Llama3 bos; got: {out:?}"
    );
    assert!(
        out.contains("<|start_header_id|>user<|end_header_id|>"),
    ...
    assert!(
        out.contains("<|start_header_id|>assistant<|end_header_id|>"),
...
fn pipeline_qwen3_chatml_no_tools() {
    ...
    let (tmpl, ctx) =
        build_context_from_gguf(&path, true).expect("failed to load Qwen3 tokenizer info");
    ...
    assert!(out.contains("Hello"), "user content; got: {out:?}");
    assert!(out.contains("<|im_end|>"), "ChatML close tag; got: {out:?}");
    assert!(
...
fn pipeline_all_available_models_render_without_panic() {
    ...
    for entry in std::fs::read_dir(&dir).unwrap().flatten() {
        ...
        let out = render_chat_template_with_context(&tmpl, &msgs, &ctx);
        assert!(!out.is_empty(), "empty output for {}", path.display());
        eprintln!(
            "  OK  {} ({} bytes)",
            path.file_name().unwrap().to_string_lossy(),
            out.len()
