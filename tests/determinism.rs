...
fn test_exact_determinism_parity() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if !Path::new(&model_path).exists() {
        println!("Skipping determinism test: model not found at {}", model_path);
        return Ok(());
    }
    ...
    for _ in 0..10 {
        ...
        out1.push_str(&tokenizer.decode_single(next as u32, true).unwrap_or_default());
    ...
    for _ in 0..10 {
        ...
        out2.push_str(&tokenizer.decode_single(next as u32, true).unwrap_or_default());
